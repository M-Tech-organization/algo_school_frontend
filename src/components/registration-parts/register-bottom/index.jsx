...
const RegisterBottom = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.link.trim()) {
      setMessage("Iltimos, F.I.Sh maydonini to'ldiring!");
      setIsLoading(false);
      return;
    }

    // Agar card to'lov tanlangan va file yuklanmagan bo'lsa
    if (paymentMethod === "card" && !uploadedFile) {
      setMessage("Iltimos, to'lov chekini yuklang!");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      // Backendda qabul qilinadigan payment_method qiymatlarini tekshirish kerak
      // "card" ishlamasa, boshqa qiymatlar bilan sinab ko'ramiz
      let backendPaymentMethod = "";
      if (paymentMethod === "card") {
        // Turli variantlarni sinab ko'ramiz
        backendPaymentMethod = "credit_card"; // yoki "bank_card", "card_payment"
      }
      submitData.append("payment_method", backendPaymentMethod);
      ...
      console.log("full_name:", formData.link);
      console.log("phone:", formData.reason);
      console.log("course_type:", formData.courseType);
      console.log("payment_method:", backendPaymentMethod);
      console.log("file:", uploadedFile ? "yuklandi" : "yo'q");
      ...
      if (!response.ok) {
        // Backenddan qaytgan xatoni ko'rsatish
  ...
  // Verify tugmasini faqat card to'lov tanlangan va file YUKLANGANDA ishlaydi
  const canSubmit = () => {
    if (isLoading) return false;
    if (!formData.link.trim()) return false;
    if (paymentMethod === "card" && !uploadedFile) return false;
    return true;
  };
                    ...
                    <div className="bg-white rounded-lg p-4 sm:p-6 md:p-8 lg:p-10">
                      ...
                      {!uploadedFile && (
                        <p className="text-red-500 text-sm mt-2">
                          To'lov chekini yuklashingiz shart!
                        </p>
                      )}
                    </div>
                  </div>
