...
const RegisterBottom = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.courseType === "Course Type") {
      setMessage("Iltimos, kurs turini tanlang!");
      setIsLoading(false);
      return;
    }

    // Agar to'lov usuli tanlanmagan bo'lsa
    if (!paymentMethod) {
    ...
    try {
      ...
      // Backend ma'lumotlariga qarab course_type "Junior" formatda kutiladi
      submitData.append("course_type", formData.courseType);

      // Payment method - backend kutgan qiymatlarni topish kerak
      // Masalan: "Card", "Click" yoki boshqa variant
      ...
      // Muvaffaqiyatli javob - /thanks sahifasiga o'tish
      navigate("/thanks");
    } catch (error) {
  ...
  // Verify tugmasini faqat barcha shart bajarilib, file yuklanganda ishlaydi
  const canSubmit = () => {
    if (isLoading) return false;
    if (!formData.link.trim()) return false;
    if (formData.courseType === "Course Type") return false;
