...
const RegisterBottom = () => {
  ...
  const validateCourseType = (courseType) => {
    if (courseType === "O'quv bosqichi" || courseType === "Course Type") {
      return "Kurs turini tanlang";
    }
    return "";
  };
  ...
  const validateFile = (file, method) => {
    ...
    if (file) {
      ...
      if (file.size > maxSize) {
        ...
        return `Fayl hajmi juda katta (${fileSizeMB} MB). Maksimal 50 MB bo'lishi kerak`;
      }
      const allowedTypes = [
        "image/jpeg",
        "image/jpg",
  ...
  const canSubmit = () => {
    return (
      ...
      formData.fullName.trim() &&
      formData.phone !== "+998" &&
      formData.courseType !== "O'quv bosqichi" &&
      formData.courseType !== "Course Type" &&
      paymentMethod &&
      (paymentMethod !== "Card" || uploadedFile) &&
      !Object.values(errors).some((error) => error !== "")
                    ...
                    <div className="bg-white rounded-lg p-4 sm:p-6 md:p-8 lg:p-[30px]">
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-3 bg-gradient-to-b from-[#F0F3FF] to-[#D9ECFF] w-full px-4 sm:px-6 md:px-8 lg:px-10 py-8 sm:py-10 md:py-12 lg:py-14 rounded-lg">
                       ...
                       block max-w-[150px] sm:max-w-[250px] md:max-w-[300px] lg:max-w-[400px]"
                            ...
                            {fileSize && (
                              <span className="text-[10px] sm:text-[12px] md:text-[13px] lg:text-[14px] text-[#666666]">
                                Hajm: {fileSize}
                              </span>
                            )}
                          </div>
