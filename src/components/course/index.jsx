    ...
    <div id="course" className=" bg-gray-50 py-[100px] px-4">
      <div className="max-w-[1000px] mx-auto">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-lg">
          <div className="flex justify-center mb-[60px]">
            <div className="inline-flex bg-white rounded-[14px] shadow-sm p-1">
              {tabs.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`px-[90px] py-[16px] rounded-[12px] font-semibold text-[18px] transition-all ${
                    activeTab === tab.id
          ...
          <div className="grid md:grid-cols-2 gap-40 items-center">
            <div>
              <h3 className="text-[32px] font-semibold text-[#262626] mb-4">
                {currentCourse.title}
              </h3>
              <div className="flex items-baseline gap-2 mb-6">
                <span className="text-[38px] line-through font-semibold text-[#FD5A5A]">
                  {currentCourse.fixed_price}
                </span>
                <span className="text-[35px]">/</span>
                <span className="text-[38px] font-bold bg-gradient-to-r from-[#6651FF] to-[#1FB3F5] bg-clip-text text-transparent">
                  {currentCourse.price}
                </span>
                <span className="text-[#4C4C4D] text-[18px] font-medium">
                ...
                {currentCourse.features.map((feature, index) => (
                  <div key={index} className="flex items-start gap-3">
                    <div className="mt-0.5 bg-[#DCF4FF] p-1 rounded-[4px]">
                      <Check className="w-5 h-5 text-[#4C4C4D]" />
                    </div>
                    <span className="text-[#4C4C4D] text-[18px] font-[400]">
                      {feature}
                    </span>
                  </div>
                ))}
              ...
              <button
                ...
                className="relative cursor-pointer my-[40px] w-full max-w-[386px] h-[60px] rounded-[10px] text-white font-semibold text-[18px] overflow-hidden transition-all duration-300 group"
              >
            ...
            <div className="bg-gray-200 rounded-2xl max-w-[380px] w-full h-64 md:h-[410px] flex items-center justify-center">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          {/* Modal o'zi */}
          <div className="relative bg-white w-[750px] h-[510px] rounded-2xl overflow-hidden flex shadow-lg z-10">
            {/* Close button top-right */}
            <button
              onClick={() => setIsModalOpen(false)}
              className="absolute top-4 right-4 z-50"
            >
              <X className="text-[#4C4C4D]" size={31} />
            </button>
            ...
            <div className="w-1/2 p-6 flex flex-col justify-center gap-4">
              ...
              <button className="w-full text-[18px] uppercase mt-6 bg-gradient-to-r from-blue-500 to-purple-600 text-white py-3 rounded-[8px] font-semibold hover:opacity-90 transition">
                Register
              </button>
            </div>
